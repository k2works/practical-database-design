...
/**
 * SchemaSpy FAS 関連のGulpタスクを登録する
 * @param {import('gulp').Gulp} gulp
 */
export default function (gulp) {
    const PROJECT_DIR = path.join(process.cwd(), 'apps', 'fas');
    const OUTPUT_DIR = path.join(process.cwd(), 'docs', 'assets', 'schemaspy-output', 'fas');
    const INDEX_FILE = path.join(OUTPUT_DIR, 'index.html');

    /**
     * バックエンドを起動してマイグレーションを実行
     */
    gulp.task('schemaspy:fas:migrate', async () => {
        console.log('Starting FAS backend for database migration...');
        ...
        console.log('\nStarting FAS backend (running Flyway migrations)...');
     ...
     */
    gulp.task('schemaspy:fas:run', (done) => {
        try {
            console.log('Running SchemaSpy for FAS...');
            ...
            console.log('\nSchemaSpy ER diagram for FAS generated successfully!');
     ...
     * バックエンドを停止する
     */
    gulp.task('schemaspy:fas:stop-backend', (done) => {
        try {
            console.log('Stopping FAS backend...');
     ...
     */
    gulp.task('schemaspy:fas:generate', gulp.series(
        'schemaspy:fas:migrate',
        'schemaspy:fas:run',
        'schemaspy:fas:stop-backend'
    ));

    /**
     * 生成されたSchemaSpy ER図をブラウザで開く
     */
    gulp.task('schemaspy:fas:open', (done) => {
        try {
            console.log('Opening SchemaSpy ER diagram for FAS...');

            // index.html が存在するか確認
            if (!fs.existsSync(INDEX_FILE)) {
                console.error(`Error: SchemaSpy output not found at ${INDEX_FILE}`);
                console.log('Please run "gulp schemaspy:fas:generate" first.');
     ...
     */
    gulp.task('schemaspy:fas:clean', (done) => {
        try {
            console.log('Cleaning SchemaSpy FAS output directory...');
            ...
            if (fs.existsSync(OUTPUT_DIR)) {
                ...
                console.log('SchemaSpy FAS output directory cleaned successfully!');
            } else {
                console.log('SchemaSpy FAS output directory does not exist.');
            }
     ...
     */
    gulp.task('schemaspy:fas:regenerate', gulp.series('schemaspy:fas:clean', 'schemaspy:fas:generate'));

    /**
     * SchemaSpy を生成してブラウザで開く
     */
    gulp.task('schemaspy:fas', gulp.series('schemaspy:fas:generate', 'schemaspy:fas:open'));

    /**
     * ヘルプを表示
     */
    gulp.task('schemaspy:fas:help', (done) => {
        console.log(`
SchemaSpy FAS Tasks
===================

Available tasks:
  schemaspy:fas:generate    - Generate SchemaSpy ER diagram (with migrations)
  schemaspy:fas:migrate     - Run database migrations only
  schemaspy:fas:run         - Run SchemaSpy only (no migrations)
  schemaspy:fas:open        - Open generated ER diagram in browser
  schemaspy:fas:clean       - Clean output directory
  schemaspy:fas:regenerate  - Clean and regenerate ER diagram
  schemaspy:fas             - Generate and open ER diagram
  schemaspy:fas:help        - Show this help
