...
/**
 * SchemaSpy PMS 関連のGulpタスクを登録する
 * @param {import('gulp').Gulp} gulp
 */
export default function (gulp) {
    const PROJECT_DIR = path.join(process.cwd(), 'apps', 'pms');
    const OUTPUT_DIR = path.join(process.cwd(), 'docs', 'assets', 'schemaspy-output', 'pms');
    const INDEX_FILE = path.join(OUTPUT_DIR, 'index.html');

    /**
     * バックエンドを起動してマイグレーションを実行
     */
    gulp.task('schemaspy:pms:migrate', async () => {
        console.log('Starting PMS backend for database migration...');
        ...
        console.log('\nStarting PMS backend (running Flyway migrations)...');
     ...
     */
    gulp.task('schemaspy:pms:run', (done) => {
        try {
            console.log('Running SchemaSpy for PMS...');
            ...
            console.log('\nSchemaSpy ER diagram for PMS generated successfully!');
     ...
     * バックエンドを停止する
     */
    gulp.task('schemaspy:pms:stop-backend', (done) => {
        try {
            console.log('Stopping PMS backend...');
     ...
     */
    gulp.task('schemaspy:pms:generate', gulp.series(
        'schemaspy:pms:migrate',
        'schemaspy:pms:run',
        'schemaspy:pms:stop-backend'
    ));

    /**
     * 生成されたSchemaSpy ER図をブラウザで開く
     */
    gulp.task('schemaspy:pms:open', (done) => {
        try {
            console.log('Opening SchemaSpy ER diagram for PMS...');

            // index.html が存在するか確認
            if (!fs.existsSync(INDEX_FILE)) {
                console.error(`Error: SchemaSpy output not found at ${INDEX_FILE}`);
                console.log('Please run "gulp schemaspy:pms:generate" first.');
     ...
     */
    gulp.task('schemaspy:pms:clean', (done) => {
        try {
            console.log('Cleaning SchemaSpy PMS output directory...');
            ...
            if (fs.existsSync(OUTPUT_DIR)) {
                ...
                console.log('SchemaSpy PMS output directory cleaned successfully!');
            } else {
                console.log('SchemaSpy PMS output directory does not exist.');
            }
     ...
     */
    gulp.task('schemaspy:pms:regenerate', gulp.series('schemaspy:pms:clean', 'schemaspy:pms:generate'));

    /**
     * SchemaSpy を生成してブラウザで開く
     */
    gulp.task('schemaspy:pms', gulp.series('schemaspy:pms:generate', 'schemaspy:pms:open'));

    /**
     * ヘルプを表示
     */
    gulp.task('schemaspy:pms:help', (done) => {
        console.log(`
SchemaSpy PMS Tasks
===================

Available tasks:
  schemaspy:pms:generate    - Generate SchemaSpy ER diagram (with migrations)
  schemaspy:pms:migrate     - Run database migrations only
  schemaspy:pms:run         - Run SchemaSpy only (no migrations)
  schemaspy:pms:open        - Open generated ER diagram in browser
  schemaspy:pms:clean       - Clean output directory
  schemaspy:pms:regenerate  - Clean and regenerate ER diagram
  schemaspy:pms             - Generate and open ER diagram
  schemaspy:pms:help        - Show this help
