...
-- 外部キー（H2の制約対応）
ALTER TABLE "倉庫マスタ" ADD CONSTRAINT IF NOT EXISTS fk_倉庫_部門
    FOREIGN KEY ("部門コード") REFERENCES "部門マスタ"("部門コード");
...
CREATE INDEX IF NOT EXISTS idx_在庫_場所 ON "在庫情報" ("場所コード");
CREATE INDEX IF NOT EXISTS idx_在庫_品目 ON "在庫情報" ("品目コード");
...
CREATE INDEX IF NOT EXISTS idx_払出指示_オーダ ON "払出指示データ" ("オーダ番号");
CREATE INDEX IF NOT EXISTS idx_払出指示_場所 ON "払出指示データ" ("場所コード");

-- 外部キー（H2の制約対応）
ALTER TABLE "払出指示データ" ADD CONSTRAINT IF NOT EXISTS fk_払出指示_オーダ
    FOREIGN KEY ("オーダ番号") REFERENCES "オーダ情報"("オーダNO");
ALTER TABLE "払出指示データ" ADD CONSTRAINT IF NOT EXISTS fk_払出指示_場所
    FOREIGN KEY ("場所コード") REFERENCES "場所マスタ"("場所コード");
...
CREATE INDEX IF NOT EXISTS idx_払出_作業指示 ON "払出データ" ("作業指示番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "払出データ" ADD CONSTRAINT IF NOT EXISTS fk_払出_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");
ALTER TABLE "払出データ" ADD CONSTRAINT IF NOT EXISTS fk_払出_場所
    FOREIGN KEY ("場所コード") REFERENCES "場所マスタ"("場所コード");
...
CREATE TABLE IF NOT EXISTS "棚卸データ" (
    ...
    "場所コード" VARCHAR(20) NOT NULL,
    "棚卸日" DATE NOT NULL,
    "ステータス" VARCHAR(20) DEFAULT '発行済' NOT NULL,
    "作成日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "作成者" VARCHAR(50),
    "更新日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "更新者" VARCHAR(50),
...
CREATE INDEX IF NOT EXISTS idx_棚卸_日付 ON "棚卸データ" ("棚卸日");

-- 外部キー（H2の制約対応）
ALTER TABLE "棚卸データ" ADD CONSTRAINT IF NOT EXISTS fk_棚卸_場所
    FOREIGN KEY ("場所コード") REFERENCES "場所マスタ"("場所コード");
...
CREATE INDEX IF NOT EXISTS idx_在庫調整_棚卸 ON "在庫調整データ" ("棚卸番号");
...
-- 受入検査結果データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "受入検査結果データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "受入検査番号" VARCHAR(20) NOT NULL,
    "欠点コード" VARCHAR(10) NOT NULL,
    "数量" DECIMAL(15, 2) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_受入検査結果_受入検査番号 ON "受入検査結果データ"("受入検査番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "受入検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_受入検査結果_受入検査
    FOREIGN KEY ("受入検査番号") REFERENCES "受入検査データ"("受入検査番号");
ALTER TABLE "受入検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_受入検査結果_欠点
    FOREIGN KEY ("欠点コード") REFERENCES "欠点マスタ"("欠点コード");

-- --------------------------------------------------
-- 工程検査データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "工程検査データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_工程検査_作業指示番号 ON "工程検査データ"("作業指示番号");
CREATE INDEX IF NOT EXISTS idx_工程検査_検査日 ON "工程検査データ"("検査日");
CREATE INDEX IF NOT EXISTS idx_工程検査_工程コード ON "工程検査データ"("工程コード");

-- 外部キー（H2の制約対応）
ALTER TABLE "工程検査データ" ADD CONSTRAINT IF NOT EXISTS fk_工程検査_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");
ALTER TABLE "工程検査データ" ADD CONSTRAINT IF NOT EXISTS fk_工程検査_工程
    FOREIGN KEY ("工程コード") REFERENCES "工程マスタ"("工程コード");

-- --------------------------------------------------
-- 工程検査結果データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "工程検査結果データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_工程検査結果_工程検査番号 ON "工程検査結果データ"("工程検査番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "工程検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_工程検査結果_工程検査
    FOREIGN KEY ("工程検査番号") REFERENCES "工程検査データ"("工程検査番号");
ALTER TABLE "工程検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_工程検査結果_欠点
    FOREIGN KEY ("欠点コード") REFERENCES "欠点マスタ"("欠点コード");

-- --------------------------------------------------
-- 出荷検査データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "出荷検査データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_出荷検査_出荷番号 ON "出荷検査データ"("出荷番号");
CREATE INDEX IF NOT EXISTS idx_出荷検査_検査日 ON "出荷検査データ"("検査日");

-- --------------------------------------------------
-- 出荷検査結果データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "出荷検査結果データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_出荷検査結果_出荷検査番号 ON "出荷検査結果データ"("出荷検査番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "出荷検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_出荷検査結果_出荷検査
    FOREIGN KEY ("出荷検査番号") REFERENCES "出荷検査データ"("出荷検査番号");
ALTER TABLE "出荷検査結果データ" ADD CONSTRAINT IF NOT EXISTS fk_出荷検査結果_欠点
    FOREIGN KEY ("欠点コード") REFERENCES "欠点マスタ"("欠点コード");

-- --------------------------------------------------
-- ロットマスタ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "ロットマスタ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_ロット_品目コード ON "ロットマスタ"("品目コード");
CREATE INDEX IF NOT EXISTS idx_ロット_製造日 ON "ロットマスタ"("製造日");
CREATE INDEX IF NOT EXISTS idx_ロット_有効期限 ON "ロットマスタ"("有効期限");

-- --------------------------------------------------
-- ロット構成（トレーサビリティ用）
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "ロット構成" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_ロット構成_親ロット番号 ON "ロット構成"("親ロット番号");
CREATE INDEX IF NOT EXISTS idx_ロット構成_子ロット番号 ON "ロット構成"("子ロット番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "ロット構成" ADD CONSTRAINT IF NOT EXISTS fk_ロット構成_親
    FOREIGN KEY ("親ロット番号") REFERENCES "ロットマスタ"("ロット番号");
ALTER TABLE "ロット構成" ADD CONSTRAINT IF NOT EXISTS fk_ロット構成_子
    FOREIGN KEY ("子ロット番号") REFERENCES "ロットマスタ"("ロット番号");
...
-- 材料消費データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "材料消費データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "作業指示番号" VARCHAR(20) NOT NULL,
    ...
    "直接材料フラグ" BOOLEAN DEFAULT true NOT NULL,
    "備考" VARCHAR(500),
    "作成日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "更新日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "バージョン" INTEGER DEFAULT 1 NOT NULL
...
CREATE INDEX IF NOT EXISTS idx_材料消費_作業指示 ON "材料消費データ"("作業指示番号");
CREATE INDEX IF NOT EXISTS idx_材料消費_消費日 ON "材料消費データ"("消費日");

-- 外部キー（H2の制約対応）
ALTER TABLE "材料消費データ" ADD CONSTRAINT IF NOT EXISTS fk_材料消費_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");
-- 品目マスタは世代管理対応（複合キー）のため、品目コードのみでの外部キー参照は不可
...
-- 賃率マスタ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "賃率マスタ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    "直接労務フラグ" BOOLEAN DEFAULT true NOT NULL,
    "作成日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "更新日時" TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
    "バージョン" INTEGER DEFAULT 1 NOT NULL,
...
-- 製造間接費マスタ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "製造間接費マスタ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_製造間接費_期間 ON "製造間接費マスタ"("会計期間");

-- --------------------------------------------------
-- 製造間接費配賦データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "製造間接費配賦データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "作業指示番号" VARCHAR(20) NOT NULL,
    "会計期間" VARCHAR(7) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_間接費配賦_作業指示 ON "製造間接費配賦データ"("作業指示番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "製造間接費配賦データ" ADD CONSTRAINT IF NOT EXISTS fk_間接費配賦_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");

-- --------------------------------------------------
-- 標準原価マスタ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "標準原価マスタ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "品目コード" VARCHAR(20) NOT NULL,
    "適用開始日" DATE NOT NULL,
    "適用終了日" DATE,
...
CREATE INDEX IF NOT EXISTS idx_標準原価_品目 ON "標準原価マスタ"("品目コード");
-- 品目マスタは世代管理対応（複合キー）のため、品目コードのみでの外部キー参照は不可

-- --------------------------------------------------
-- 実際原価データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "実際原価データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "作業指示番号" VARCHAR(20) NOT NULL,
    "品目コード" VARCHAR(20) NOT NULL,
    "完成数量" DECIMAL(15, 2) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_実際原価_作業指示 ON "実際原価データ"("作業指示番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "実際原価データ" ADD CONSTRAINT IF NOT EXISTS fk_実際原価_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");
-- 品目マスタは世代管理対応（複合キー）のため、品目コードのみでの外部キー参照は不可
...
-- 原価差異データ
-- --------------------------------------------------
CREATE TABLE IF NOT EXISTS "原価差異データ" (
    "ID" INT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    "作業指示番号" VARCHAR(20) NOT NULL,
    "品目コード" VARCHAR(20) NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_原価差異_作業指示 ON "原価差異データ"("作業指示番号");

-- 外部キー（H2の制約対応）
ALTER TABLE "原価差異データ" ADD CONSTRAINT IF NOT EXISTS fk_原価差異_作業指示
    FOREIGN KEY ("作業指示番号") REFERENCES "作業指示データ"("作業指示番号");
-- 品目マスタは世代管理対応（複合キー）のため、品目コードのみでの外部キー参照は不可
